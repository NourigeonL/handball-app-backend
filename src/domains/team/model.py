...
from src.domains.team.entities import TeamPlayer
from src.domains.team.events import PlayerAdded, PlayerRemoved, TeamCreated
...
class Team(AggregateRoot):
    ...
    def __init__(self, init: TeamInit | None = None, actor_id: str | None = None):
        super().__init__()
        self.players : dict[str, TeamPlayer] = {}
        self.nb_players_by_position : dict[PlayerPosition, int] = {}
        if init:
            self._apply_change(TeamCreated(team_id=init.team_id, category=init.category, club_id=init.club_id, name=init.name, gender=init.gender, season=init.season or get_current_season(), actor_id=actor_id))
    ...
    def add_player(self, player: TeamPlayer, actor_id: str) -> None:
        ...
        self._apply_change(PlayerAdded(team_id=self.__id, player=player, actor_id=actor_id))

    def remove_player(self, license_id: str, actor_id: str) -> None:
        if not self.players.get(license_id):
            raise InvalidOperationError(f"Player with license id {license_id} does not exist")
        self._apply_change(PlayerRemoved(team_id=self.__id, player_id=license_id, actor_id=actor_id))

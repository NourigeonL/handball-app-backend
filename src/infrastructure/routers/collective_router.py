...
@router.post("/create")
async def create_collective(collective_create_data: CollectiveCreateRequest, current_user: UserSession = Depends(get_current_user)):
...
async def add_player_to_collective(collective_id: str, add_player_to_collective_data: AddPlayerToCollectiveRequest, current_user: UserSession = Depends(get_current_user)):
    await service_locator.collective_service.handle(AddPlayerToCollectiveCommand(actor_id=current_user.user_id, collective_id=collective_id, player_id=add_player_to_collective_data.player_id))
...
async def remove_player_from_collective(collective_id: str, remove_player_from_collective_data: RemovePlayerFromCollectiveRequest, current_user: UserSession = Depends(get_current_user)):
    await service_locator.collective_service.handle(RemovePlayerFromCollectiveCommand(actor_id=current_user.user_id, collective_id=collective_id, player_id=remove_player_from_collective_data.player_id))
